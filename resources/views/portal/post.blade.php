	...
	<div class="row">
		...
		<div class="col-md-6">
			<div class="recomend">
				...
				<div class="post-info">
					<a class="btn caegory-btn" href="#">{{$posts->name}}</a>
					<h5 class="title"><a href="{{url('post-detail/'.$posts->id)}}"><b class="light-color">{{substr($posts->title, 0, 30).(strlen($posts->title)>30?"...":"")}}</b></a></h5>
					<h6 class="date"><em>{{date('D, M Y', strtotime($posts->created_at))}}</em></h6>{!!substr($posts->content, 0, 30).(strlen($posts->content)>30?"...":"")!!}
				</div>
			</div>
		</div>
		@endforeach
